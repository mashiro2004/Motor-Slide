...
<div style="background-color:#f1f1f1;padding:15px;text-align:center;">
 ...
 <div class="main">
	...
	<h2>Posizionamento Passo Passo:</h2>
    <form method="post">
    ...
    <input type="text" id="scatti" name="scatti" value="<?php if (isset($_POST['scatti'])) echo $_POST['scatti']; ?>"/><br><br>
	<label for="lblsec">Secondi tra uno scatto e l'altro:</label>
    <input type="text" id="sec" name="sec" value="<?php if (isset($_POST['sec'])) echo $_POST['sec']; ?>"/><br><br>
	<label for="lblsec">Numero di step tra uno scatto e l'altro:</label>
    <input type="text" id="rot" name="rot" value="<?php if (isset($_POST['rot'])) echo $_POST['rot']; ?>"/><br><br>
...
//Alla pressione di Orario del posizionamento
if (isset($_POST['avanti']))
{
set_time_limit(300);
	// Loop fino allo stop
for($i=1; $i<=65500; $i++){
	//Leggo il pin 21 del fincorsa
	$file = fopen("/sys/class/gpio/gpio21/value","r");
	$fcorsa = fgets($file);
	fclose($file);
	  if ((isset($_POST['stop'])) or ($fcorsa == 1))
	  {
		  break;
	  }
...
//Alla pressione di Antiorario del posizionamento
...
// Alla pressione micrometrica antiorario
